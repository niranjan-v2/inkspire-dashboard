...
const App = () => {
  ...
  const projects = [
    ...
    },
    {
      ...
      description: 'Web interface for exploring transactions on a custom blockchain.',
      longDescription: 'React frontend for visualizing blocks, transactions, and wallet balances. Features real-time updates via WebSocket and address tagging system. Includes transaction graph visualization using D3.js.',
    ...
    <div className="max-w-full mx-auto px-6 py-8 font-mono bg-gray-50 dark:bg-[#0a192f] text-gray-800 dark:text-[#ccd6f6] transition-colors duration-300">
      <header className="mb-12 relative">
        <div className="bg-gray-200 dark:bg-[#1e2a47] px-4 py-2 rounded-t-lg flex items-center mb-[-1px] justify-between">
          ...
          <span className="ml-4 text-sm text-gray-600 dark:text-[#8892b0]">portfolio_terminal -- user@dev: ~/projects</span>
        </div>
        ...
        <p className="text-gray-600 dark:text-[#8892b0] italic mb-8 text-sm">// Built with thought, purpose and ♥</p>
      </header>
      ...
      <div className="flex flex-col gap-6">
        {filteredProjects.map(project => (
          <div 
            key={project.id} 
            className={`bg-white dark:bg-[#112240] rounded-lg p-6 border transition-all ${
              expandedProject === project.id 
                ? 'border-blue-400 dark:border-[#64ffda]' 
                : 'border-gray-200 dark:border-[rgba(100,255,218,0.2)]'
          ...
          >
            ...
            >
              <h3 className="text-xl font-semibold text-blue-600 dark:text-[#00ff9d] m-0">{project.title}</h3>
            ...
            <p className="text-gray-600 dark:text-[#8892b0] mb-4 text-sm">{project.description}</p>
            
            {expandedProject === project.id && (
              <div>
                <p className="text-gray-600 dark:text-[#8892b0] my-4 text-sm leading-relaxed">{project.longDescription}</p>
              </div>
            )}
      ...
      <footer className="text-center mt-12 text-gray-600 dark:text-[#8892b0] text-sm pt-8 border-t border-gray-200 dark:border-[#8892b0]/20">
        <div className="mb-4 flex justify-center items-center">
          <span className="text-blue-600 dark:text-[#00ff9d] mr-2">$</span>
          <span className="text-gray-800 dark:text-[#ccd6f6]">cat about_me.txt</span>
        </div>
