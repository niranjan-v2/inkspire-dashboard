...
export default function PostPage() {
  ...
  return (
    <main className="p-3 flex flex-col max-w-7xl mx-auto min-h-screen">
      <h1 className="text-3xl mt-10 p-3 text-center font-serif max-w-4xl mx-auto lg:text-5xl">
        {post && post.title}
      </h1>
      <Link
      ...
      />
      <div className="flex justify-between p-3 border-b border-slate-300 mx-auto w-full max-w-4xl text-sm">
        <span>{post && new Date(post.createdAt).toLocaleDateString()}</span>
        <span className="italic">
          {post && (post.content.length / 1000).toFixed(0)} mins read
        </span>
      </div>
      <div className="p-3 w-full max-w-4xl mx-auto prose prose-lg">
        <article 
